RSpec.describe "Testing main functionality ", :type => :request do
  ...
  describe "nodes main functionality (uninstall node)" do
    ...
    before(:all) do
      ...
      @node_uid = ENV['node_name'] || @user_data['aws']["#{@cluster_name}_data"]["#{@node_name}_data"]['node_uid']
    ...
    after(:each) do |example|
      ...
      slack_msg_uninstall_node(passed, @user_name, 'AWS CORE', @cluster_name, @cluster_id, @cluster_uid, @node_name, @node_uid, test_exception)
    end


    it 'uninstall node'  do

      puts 'UNINSTALL AWS CORE NODE'
      ...
      @user_data['aws']["#{@cluster_name}_data"].delete("node_name")
      @user_data['aws']["#{@cluster_name}_data"].delete("#{@node_name}_data")
