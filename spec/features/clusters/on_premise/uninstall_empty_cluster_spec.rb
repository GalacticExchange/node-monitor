RSpec.describe "Testing main functionality ", :type => :request do
  ...
  describe "Cluster" do
    before(:all) do
      ...
      @cluster_uid = ENV['cluster_uid'] || @user_data['on_premise']["#{@cluster_name}_data"]['cluster_uid']
    ...
    it 'delete cluster' do
      ...
      cluster_info = {}
      cluster_info = @user_data['on_premise']
      @user_data['on_premise'].delete("cluster_name")
      @user_data['on_premise'].delete("#{@cluster_name}_data")
      @user_data.delete('on_premise') if cluster_info.size == 0
