RSpec.describe "Testing install app", :type => :request do
  ...
  describe "apps installation" do
    ...
    it 'rocana installation'  do
      ...
      installed_apps_tab.click
      sleep 3
      find('[data-row="app-rocana"]').click
      ...
      for i in 0...services_name.size
        for k in 1..20
          ...
          if stdout =~ /Connected/
            ...
            puts k
            sleep 60
            puts "STDOUT: #{stdout}" if k == 20
            fail "Failed to connect to service #{services_name[i]}" if k == 20
          else
            puts "STDOUT: #{stdout}"
